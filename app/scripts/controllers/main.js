...
angular.module('countytraceApp')
    .controller('MainCtrl', function($scope, $rootScope, $localStorage, $cookies, ProjectsService, BlockService, ConcernsService) {
        $scope.displayInfo = {};
        $scope.gridOptions = {
            enableSorting: true,
        ...
        $scope.$watch("newConcern.level", function(newValue, oldValue) {
          if($scope.newConcern.level=='country'){
            $scope.blockList=[{'name':'Kenya', 'blockId':'48'}];
            $scope.addresseeList=[{"name":"President",'value':'0'},{'name':"Deputy President", 'value':'1'}];
          }
          else if($scope.newConcern.level=='none'){
            $scope.blockList=[{'name':'Block Name', 'blockId':''}];
            $scope.addresseeList=[{"name":"Select a Block Level First", 'value':'0'}];
          }
          else{
            $scope.getBlockList();
            if($scope.newConcern.level=='county'){
              $scope.addresseeList=[{"name":"Governor",'value':'0'},{'name': "Deputy Governor", 'value':'1'}];
            }
            else if($scope.newConcern.level=='constituency'){
              $scope.addresseeList=[{'name':"M.P.",'value':'0'},{'name': "C.D.F. Chairperson", 'value':'1'}];
            }
            else{
              $scope.addresseeList=[{'name':"M.C.A", 'value':'0'}];
            }
          }
        });
        ...
        $scope.$watch("newProject.level", function(newValue, oldValue) {
          if($scope.newConcern.level=='country'){
            ...
            $scope.blockList=[{'name':'Block Name', 'blockId':''}];
            $scope.addresseeList=[{"name":"Select a Block Level", 'value':'0'}];
          }
          else{
            $scope.getBlockList();
            if($scope.newConcern.level=='county'){
        ...
        $scope.getBlockList=function(){
            ...
            list.$promise.then(function(successResp){
              angular.forEach(successResp, function(value, key){
                var data={'name':value.name, 'blockId': value.blockId};
